...
- **开盘后 1 小时内的放宽**：30/60 分钟窗口在开盘初期天然不足（60 分钟窗口约需累积 49 个分钟点才 >80%），因此开盘后 1 小时内只校验 **5/10 分钟**窗口；数据参考仍尽量覆盖 5/10/30/60（指标照常按四个周期计算，10/30/60 分钟线有历史预热时趋势过滤不受影响）。`dataCoverage` 仍报告全部四个窗口，仅 `reasonCodes`/拦截按生效窗口输出。
