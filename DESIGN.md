...
| 问题 | 确认结果 |
| --- | --- |
| 信号金价标的 | **上金所 Au99.99**（SGE，元/克） |
| 实际交易标的 | **招商银行积存金**；插件用 Au99.99 做信号，招行价按固定价差自动估算（默认 +1.72 元/克，买卖价差可分别设置） |
...
- 页面内轻量浮窗看板：
  ...
  - 展开态小卡：Au99.99 5 分钟趋势线（SVG）、国际价折算元/克、内外价差、招行积存金估算价（Au99.99 + 固定价差）、当前信号摘要。
...
- 设置页（`settings.section`）：
  ...
  - 招行积存金价差：买入价差、卖出价差默认各 +1.72 元/克，可分别调整。
  - 信号阈值、交易时段（工作日 09:00–次日 02:00，节假日表可编辑）。
  - 系统通知开关、Webhook 渠道配置（密钥只写、读回脱敏）。
- 策略输出：
  - 无持仓时：建议买入价（Au99.99 信号价 + 对应招行估算价）、建议克数、理由标签（技术指标状态）。
...
3. **信号只在 Au99.99 上计算**，实际执行的招行积存金价格用“Au99.99 + 可配置价差”估算；国际 XAU 用于趋势确认、价差显示和波动环境判断，避免“看着国际盘的价格交易国内标的”的错位。
...
| 用途 | 主源 | 备用源 | 说明 |
| --- | --- | --- | --- |
| Au99.99 报价 | 新浪 `hq.sinajs.cn/list=gds_AU9999` | 东财 `push2.eastmoney.com/api/qt/stock/get?secid=118.AU9999` | 新浪需 `Referer: https://finance.sina.com.cn/`，GBK 解码；东财 JSON，价格字段按 100 倍缩放（实测 `f43=95000` = 950.00） |
| XAU 伦敦现货 | 新浪/腾讯 `hf_XAU` | 东财 `122.XAU`（限流时降级） | GBK 解码；腾讯需 GBK→UTF-8 |
...
- 每源独立超时（5s）、重试（指数退避 2s/4s/8s，最多 3 次）、熔断（连续 5 次失败停用 10 分钟）。
...
- 招行积存金估算价：
  - `cmbBuy ≈ Au99.99 买价 + cmb.buySpreadPerGram`（默认 +1.72）
  - `cmbSell ≈ Au99.99 卖价 + cmb.sellSpreadPerGram`（默认 +1.72）
  - 价差只是估算；看板标注“招行价以 App 为准”。
...
计算顺序固定：先补足 bars → 计算指标 → 快照输出。任何指标因数据不足不输出信号，只显示 `数据积累中`。
...
- 信号价始终基于 Au99.99；展示与委托建议同时给出 `signalPrice` 和 `cmbEstimatedPrice` 两个数字。
...
2. 60 分钟 EMA20 向上（趋势过滤）；
3. 5 分钟 RSI14 从 < 35 回升并上穿 35，或 5 分钟收盘重新站上 SMA20；
4. 价格距离近 20 根 bar 低点或布林下轨不超过 0.5%。

输出：

- `action = buy_setup`
- 建议买入价（Au99.99）= `min(现价+0.1, 近20低点 + 0.3×ATR)`
...
```json
{
  ...
  "derived": {
    ...
    "cmb": { "buyPrice": 951.72, "sellPrice": 951.72, "sourceNote": "Au99.99 + 1.72 元/克估算" }
  },
  "trend": {
    "AU9999_5m": [ { "t": "...", "o": 950.0, "h": 951.0, "l": 949.2, "c": 950.4 } ],
    "XAU_5m": [ "...自建 bars..." ]
  },
  "indicators": { "AU9999_5m": { "rsi14": 46.2, "ema20": 949.8, "upper": 953.1, "lower": 946.6 } },
  "position": { "grams": 20, "avgCostPerGram": 945.0, "marketValue": 19000, "feeAdjustedPnl": 87.0 },
  "plan": { "action": "sell_take_profit", "suggestedOrder": { "...": "..." } },
...
### 7.4 `GET /dsh-plugin-goldboard/bars?instrument=AU9999&interval=5m&limit=120`
...
- 展开态小卡（约 320×220）：
  ...
  - 中部：Au99.99 5 分钟 sparkline（手写 SVG，不引图表库）；
...
2. 交易标的：实际交易**招行积存金**；插件信号用 Au99.99，招行价按固定价差估算（默认 +1.72 元/克，买卖可分别调）。
