...
export function aggregateReplayReport({
  ...
  const realBidAskCoverage = Number(diagnostics.bars) > 0
    ? rate(Number(diagnostics.realBidAskBars) || 0, Number(diagnostics.bars))
    : null;
  const caveats = [
    ...profile.caveats,
...
export function replayContinuousTradingDay(day, rawBars5All, rawBars60All, config, state, options = {}) {
  ...
  if (!account.executionDiagnostics || typeof account.executionDiagnostics !== "object") {
    account.executionDiagnostics = { bars: 0, realBidAskBars: 0, syntheticBars: 0, ambiguousBars: 0 };
  }
  let steps = 0;
  let blockedSteps = 0;
  const trades = [];
  ...
  for (const bar of dayBars5) {
    ...
    if (path.synthetic === true) account.executionDiagnostics.syntheticBars += 1;
...
export function createReplayStats(deps) {
  ...
  let lastResult = null; // { params, generatedAtMs, report, events, trades, orders, fills }
  ...
  async function run({
    ...
    const computation = (async () => {
      ...
      const continuous = {
        ...
        pendingOrders: [],
        equityCurve: [],
        executionDiagnostics: { bars: 0, realBidAskBars: 0, syntheticBars: 0, ambiguousBars: 0 },
        lastMarkPrice: null,
      };
      let steps = 0;
      let blockedSteps = 0;
