...
export const REPLAY_DATA_SCHEMA_VERSION = MARKET_DATA_SCHEMA_VERSION;
/** Default CMB lane calendar; individual reports project the selected lane version. */
export const REPLAY_CALENDAR_VERSION = calendarVersionForInstrument("CMB");
...
export function sessionDateForTimestamp(timestamp, config, options = {}) {
  const instrument = options.instrument ?? (options.lane === "au9999" ? "AU9999" : "CMB");
  return tradingDayForTimestamp(instrument, timestamp, config);
}

/** Return the selected lane's versioned session bounds for a Beijing day. */
export function replaySessionBounds(day, config, options = {}) {
  const cfg = normalizeConfig(config);
  const instrument = options.instrument ?? (options.lane === "au9999" ? "AU9999" : "CMB");
  const calendar = getTradingCalendar(instrument, cfg);
  const midnight = Date.parse(`${day}T00:00:00+08:00`);
  const openMin = Number(calendar.reportOpenMin ?? calendar.openMin);
  const closeMin = Number(calendar.reportCloseMin ?? calendar.closeMin);
  const openMs = midnight + openMin * 60_000;
  const closeMs = midnight + closeMin * 60_000;
  return {
    day,
    openMs,
    closeMs,
...
export function replaySessionStatus(day, bars5, config, asOf = new Date(), options = {}) {
  const instrument = options.instrument ?? (options.lane === "au9999" ? "AU9999" : "CMB");
  const calendar = getTradingCalendar(instrument, config);
  const bounds = replaySessionBounds(day, config, { instrument });
  const asOfMs = new Date(asOf).getTime();
  const list = (Array.isArray(bars5) ? bars5 : [])
    .filter((bar) => bar && Number.isFinite(Number(bar.t)) && sessionDateForTimestamp(Number(bar.t), config, { instrument }) === day)
    .filter((bar) => isOpenMinute(calendar, Number(bar.t)))
    .filter((bar) => bar.partial !== true)
...
export function listReplayTradingDays(config, days, now = new Date(), options = {}) {
  ...
  const todaySession = sessionDateForTimestamp(new Date(now).getTime(), config, { instrument });
  ...
  for (let offset = 0; offset < 400 && out.length < requested; offset += 1) {
    ...
    const bounds = replaySessionBounds(date, config, { instrument });
...
export function synthCmbReplayQuote(price, atMs) {
  ...
  return {
    ...
    customerAsk: buy,
    customerBid: sell,
    source: "cmb-replay-proxy",
    synthetic: true,
    quality: "proxy",
    executionSideComplete: false,
    executionEvidence: { mode: "proxy", askSource: "persisted-cmb-ask", bidSource: "fixed-spread-assumption" },
    assumptions: ["replay-cmb-fixed-spread-proxy"],
    updatedAt: atMs,
    receivedAt: new Date(atMs).toISOString(),
    staleAfterMs: 15 * 60_000,
  };
...
export function computeForwardOutcome(event, futureBars, config, sessionEndMs, options = {}) {
  ...
  const pathFor = (bar) => pathByTime?.get(Number(bar.t)) ?? resolveExecutionBar({
    ...
    assumptionSource: "replay-cmb-fixed-spread-proxy",
...
export function aggregateReplayReport({
  ...
  return {
    ...
    calendarVersion: calendarVersionForInstrument(lane === "cmb" ? "CMB" : "AU9999"),
...
export function replayTradingDay(day, rawBars5All, rawBars60All, config, options = {}) {
  ...
  const selectedLane = REPLAY_LANES.includes(options.lane) ? options.lane : "au9999";
  const profile = laneProfile(selectedLane);
  const calendarInstrument = profile.instrument === "CMB" ? "CMB" : "AU9999";
  const calendar = getTradingCalendar(calendarInstrument, cfgBase);
  // A direct pure replay call has no external as-of clock, so treat the
  // selected calendar close as its analysis instant. The engine passes its
  // real clock explicitly and can classify a current session as partial.
  const requestedBounds = replaySessionBounds(day, cfgBase, { instrument: calendarInstrument });
  ...
  const session = replaySessionStatus(day, bars5All, cfgBase, asOf, { instrument: calendarInstrument });
  const inSession = (bar) => {
    if (!bar || !Number.isFinite(bar.t) || bar.partial === true) return false;
    return sessionDateForTimestamp(bar.t, cfgBase, { instrument: calendarInstrument }) === day
      && isOpenMinute(calendar, bar.t)
      && Number(bar.t) >= session.openMs
      && Number(bar.t) < session.closeMs;
  };
...
function replayExecutionPath(bar, profile, config) {
  return resolveExecutionBar({
    ...
    allowProxy: profile.instrument === "CMB",
    assumptionSource: profile.instrument === "CMB" ? "replay-cmb-fixed-spread-proxy" : "replay-reference-bar-offset-proxy",
...
function replayQuoteForBar(bar, profile, atMs, config) {
  ...
  return {
    ...
    quality: path.realBidAsk === true ? "primary" : "proxy",
    executionSideComplete: path.realBidAsk === true,
    executionEvidence: {
      mode: path.evidence ?? (path.realBidAsk === true ? "real" : "proxy"),
      askSource: path.source ?? null,
      bidSource: path.realBidAsk === true ? path.source ?? null : "replay-cmb-fixed-spread-proxy",
    },
    updatedAt: atMs,
    receivedAt: new Date(atMs).toISOString(),
    staleAfterMs: 15 * 60_000,
...
export function replayContinuousTradingDay(day, rawBars5All, rawBars60All, config, state, options = {}) {
  ...
  const bars5All = [...(Array.isArray(rawBars5All) ? rawBars5All : [])].sort((a, b) => a.t - b.t);
  const bars60All = [...(Array.isArray(rawBars60All) ? rawBars60All : [])].sort((a, b) => a.t - b.t);
  const bounds = replaySessionBounds(day, cfgBase, { instrument: calendarInstrument });
  const asOfMs = options.asOf === undefined ? bounds.closeMs : new Date(options.asOf).getTime();
  const session = replaySessionStatus(day, bars5All, cfgBase, new Date(asOfMs), { instrument: calendarInstrument });
  const dayBars5 = bars5All.filter((bar) => bar && Number.isFinite(bar.t)
    && bar.partial !== true
    && sessionDateForTimestamp(bar.t, cfgBase, { instrument: calendarInstrument }) === day
    && isOpenMinute(calendar, bar.t)
    && bar.t >= bounds.openMs
...
export function createReplayStats(deps) {
  ...
  async function run({
    ...
    const configFingerprint = JSON.stringify({
      ...
      strategyVersion: REPLAY_STRATEGY_VERSION,
      dataSchemaVersion: REPLAY_DATA_SCHEMA_VERSION,
      calendarVersion: calendarVersionForInstrument(requestedLane === "cmb" ? "CMB" : "AU9999", configNow),
      executionVersion: EXECUTION_MODEL_VERSION,
      fillPolicy: REPLAY_FILL_POLICY,
      ambiguityPolicy: REPLAY_AMBIGUITY_POLICY,
