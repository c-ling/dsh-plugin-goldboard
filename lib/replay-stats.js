 ...
 *
 * v4 wire additions: `report.overall` is a continuous account replay: it starts
 * from zero grams, applies each simulated order in chronological order under
 * the configured maximum grams and fees, then marks any remaining position to
 * the final executable sell price. `report.signalQuality` retains the legacy
 * independent-event aggregate for the per-action diagnostic tables only.
...
export const REPLAY_STATS_VERSION = 4;
...
export function executeReplayTrade(portfolio, plan, config, fields = {}) {
  ...
  const feePerGram = (side === "buy" ? cfg.fee.buyPerGram : cfg.fee.sellPerGram) + cfg.strategy.slippagePerGram;
  const feeCny = round2(feePerGram * grams);
  const grossCny = round2(price * grams);
  const cashFlowCny = side === "buy" ? -round2(grossCny + feeCny) : round2(grossCny - feeCny);
  const next = { ...source };
...
function summarizeContinuousPortfolio(portfolio, trades, lastMarkPrice, config) {
  const cfg = normalizeConfig(config);
  const grams = Math.max(0, Number(portfolio?.grams) || 0);
  const costBasisCny = Math.max(0, Number(portfolio?.costBasisCny) || 0);
  ...
  const finalSellFeePerGram = cfg.fee.sellPerGram + cfg.strategy.slippagePerGram;
  const endingLiquidationValueCny = grams === 0
    ? 0
    : Number.isFinite(markPrice) && markPrice > 0
      ? round2(grams * Math.max(0, markPrice - finalSellFeePerGram))
      : null;
 ...
 */
export function aggregateReplayReport({ events, continuous, steps, blockedSteps, daysRequested, daysEvaluated, daysFailed, failures, params, generatedAt, window, daysSkippedNoData = 0 }) {
  const perAction = [];
...
export function replayTradingDay(day, rawBars5All, rawBars60All, config, options = {}) {
  ...
  for (const pass of ["flat", "hold"]) {
    ...
    for (const bar of dayBars5) {
      ...
      steps += 1;
      runtime.quotes[feedKey] = profile.quote(bar.c, stepNowMs);
...
/**
 * Replay one day into a single account that persists across the whole window.
 * Unlike `replayTradingDay`, this evaluates one runtime and updates its
 * simulated position after every filled order, beginning with exactly zero
 * grams regardless of the user's live lots.
 */
export function replayContinuousTradingDay(day, rawBars5All, rawBars60All, config, state, options = {}) {
  ...
  const dayBars5 = bars5All.filter((bar) => bar && Number.isFinite(bar.t)
    && sessionDateForTimestamp(bar.t, cfgBase) === day && isOpenMinute(calendar, bar.t));
  const minuteBars = expandBarsToMinuteBars(bars5All).map((bar) =>
    profile.instrument === "CMB" ? { ...bar, source: "replay-coverage-cmb", instrument: "CMB_ACCUMULATED_GOLD", market: "bank", currency: "CNY", unit: "gram" } : bar,
  );
  ...
  let steps = 0;
  let blockedSteps = 0;
  if (dayBars5.length === 0) return { steps, blockedSteps, trades: [] };
  ...
  for (const bar of dayBars5) {
    ...
    if (plan.signalState) runtime.signalState = plan.signalState;
    if (plan.action === "data_incomplete") blockedSteps += 1;
    const markPrice = Number(plan.cmbEstimatedPrice);
    if (Number.isFinite(markPrice) && markPrice > 0) account.lastMarkPrice = markPrice;
    if (!REPLAY_TRACKED_ACTIONS.includes(plan.action) || !plan.suggestedOrder) continue;
    const filled = executeReplayTrade(account.portfolio, plan, stepConfig, { t: timestamp, day });
    if (filled.trade === null) continue;
    account.portfolio = filled.portfolio;
    account.trades.push(filled.trade);
    trades.push(filled.trade);
  }
  account.signalState = runtime.signalState;
  return { steps, blockedSteps, trades };
}
...
export function createReplayStats(deps) {
  ...
  let running = null;
  let lastResult = null; // { params: {days, lane}, generatedAtMs, report, events, trades }
  ...
  async function fetchDayBars(lane, day) {
    if (lane === "cmb") {
      const series = await resolveCmbSeries();
      const { bars5, bars60 } = sliceCmbDaySeries(series.bars5, series.bars60, day, getConfig());
      return { bars5, bars60 };
    }
  ...
  async function run({ days = REPLAY_DAYS_DEFAULT, force = false, detail = false, signal = null, now = null, lane = REPLAY_LANE_DEFAULT } = {}) {
    ...
    // Config-aware cache key (v3): reports are computed under the LIVE
    ...
    const configFingerprint = JSON.stringify({ fee: configNow.fee, limits: { maxGrams: configNow.limits.maxGrams }, strategy: configNow.strategy });
    ...
    running = (async () => {
      ...
      for (const [dayIndex, day] of dayList.entries()) {
        ...
        try {
          const { bars5, bars60 } = await withAbortSignal(fetchDayBars(requestedLane, day), signal);
          const dayResult = replayTradingDay(day, bars5, bars60, config, { lane: requestedLane });
          replayContinuousTradingDay(day, bars5, bars60, config, continuous, { lane: requestedLane });
          steps += dayResult.steps;
          blockedSteps += dayResult.blockedSteps;
          events.push(...dayResult.events);
          if (dayResult.steps > 0) daysEvaluated += 1;
          else daysSkippedNoData += 1;
        } catch (error) {
      ...
      lastResult = { params: { days: requestedDays, lane: requestedLane, configFingerprint }, generatedAtMs, report, events, trades: continuous.trades };
      if (file) {
        await writeJsonAtomic(file, { report, trades: continuous.trades }, writeQueue).catch((error) => {
  ...
  async function last(detail = false) {
    ...
    if (file) {
      ...
      if (saved && typeof saved === "object" && saved.report && typeof saved.report === "object") {
        ...
        return { ok: true, cached: true, report: saved.report, ...(detail && trades ? { trades } : {}) };
      }
    }
