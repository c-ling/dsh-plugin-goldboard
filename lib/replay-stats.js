...
export const REPLAY_STATS_VERSION = 2;
 ...
 */
export function aggregateReplayReport({ events, steps, blockedSteps, daysRequested, daysEvaluated, daysFailed, failures, params, generatedAt, window, daysSkippedNoData = 0 }) {
  const perAction = [];
...
export function createReplayStats(deps) {
  ...
  let running = null;
  let lastResult = null; // { params: {days, lane}, generatedAtMs, report, events }
  ...
  async function run({ days = REPLAY_DAYS_DEFAULT, force = false, detail = false, signal = null, now = null, lane = REPLAY_LANE_DEFAULT } = {}) {
    ...
    running = (async () => {
      ...
      const report = aggregateReplayReport({
        ...
        failures,
        params: { days: requestedDays, lane: requestedLane },
        generatedAt: generatedAtMs,
        window: { from: dayList[0] ?? null, to: dayList[dayList.length - 1] ?? null },
        daysSkippedNoData,
      });
      lastResult = { params: { days: requestedDays, lane: requestedLane }, generatedAtMs, report, events };
      if (file) {
