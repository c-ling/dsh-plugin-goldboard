...
/** msToClose below which holders are nudged to close by session end. */
...
function buildPlanSeries(runtime, lc, now) {
  ...
  return {
    ...
    estSpread, slippage, fee, pnlSellFee,
    breakevenBuyFee: fee.buyPerGram,
    breakevenSellFee: fee.sellPerGram,
...
function positionBranch(base, lc, s, c, now) {
  const { cfg, liveCmb } = lc;
  const pos = cfg.position;
  const strategy = cfg.strategy;
  const lots = Array.isArray(pos.lots) ? pos.lots.filter((lot) => lot.status !== "closed") : [];
  const avgCost = pos.avgCostPerGram || 0;
  const exitNeeded = round2(avgCost + s.breakevenSellFee + s.estSpread + s.slippage);
  const cmbNow = s.cmbSell;
  const pnl = round2((cmbNow - s.pnlSellFee - avgCost) * pos.grams);
  base.position = {
    grams: pos.grams,
    ...
    lots: lots.map((lot) => ({
      ...
      time: lot.time,
      cmbNow,
      feeAdjustedPnl: round2((cmbNow - s.pnlSellFee - lot.price) * lot.grams),
      profitPerGram: round2(cmbNow - s.pnlSellFee - lot.price),
    })),
  };
  ...
  base.stopPrice = round2(avgCost - strategy.maxLossPerGram - s.pnlSellFee);
  ...
  } else if (lc.market.msToClose <= SESSION_END_WINDOW_MS) {
...
function flatBranch(base, lc, s, c, now) {
  ...
  if (c.buySetup && c.confidenceScore >= strategy.scoreThreshold) {
    ...
    const suggestedSignalPrice = s.toSignalPrice(suggestedTechnicalPrice);
    const suggestedCmbPrice = round2(suggestedSignalPrice + s.spreadCmb.buySpreadPerGram);
    const entryCost = round2(suggestedCmbPrice + s.breakevenBuyFee);
    const breakeven = round2(entryCost + s.breakevenSellFee + s.estSpread + s.slippage);
    const grams = suggestedGrams(lc.cfg);
    const targetPrice = Math.max(
  ...
  base.action = "wait";
  base.breakeven = round2(s.cmbBuy + s.breakevenBuyFee + s.breakevenSellFee + s.estSpread + s.slippage);
...
export function computePlan(runtime, config, now = new Date()) {
  ...
  const { xau, usdcny, domestic } = lc.quotes;
  ...
  const quoteStale = !lc.liveCmb && (
    (lc.useXauSignal && (xauStale || usdcnyStale)) ||
    (lc.useDomesticSignal && domesticStale)
  );
  if (quoteStale) {
    base.action = "data_stale";
    base.reasonCodes.push("stale_quote");
  ...
  s.signals = signals;
  base.confidenceScore = signals.confidenceScore;
  base.confidenceMax = CONFIDENCE_MAX;
