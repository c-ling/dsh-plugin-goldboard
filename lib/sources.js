...
const eastmoneyKlineUrl = (host, secid, klt, limit, endDate) =>
  `https://${host}/api/qt/stock/kline/get?secid=${secid}&klt=${klt}&fqt=1&lmt=${limit}` +
  `&end=${endDate}&iscca=1&fields1=f1,f2,f3,f4,f5,f6,f7,f8&fields2=f51,f52,f53,f54,f55,f56,f57,f58`;

export const EASTMONEY_KLINE_URL = (secid, klt, limit, endDate = "20500101") =>
  eastmoneyKlineUrl("push2his.eastmoney.com", secid, klt, limit, endDate);
/**
 * Eastmoney's anti-scraping mirror: when the primary host answers with empty
 * replies / connection drops (observed in production), the delayed host
 * serves identical rows for CLOSED history — for backfill and replay
 * statistics the 15-minute delay is irrelevant because those bars already
 * ended days ago. It keeps its own circuit-breaker id (`<id>-delayed`).
 */
export const EASTMONEY_DELAYED_KLINE_URL = (secid, klt, limit, endDate = "20500101") =>
  eastmoneyKlineUrl("push2delay.eastmoney.com", secid, klt, limit, endDate);
...
export const DATA_SOURCES = Object.freeze([
  ...
  Object.freeze({ id: "eastmoney-kline-delayed", nameZh: "东方财富 K线延迟镜像", nameEn: "Eastmoney Delayed Mirror K-line", source: "eastmoney", kind: "history", instrument: null, url: "https://push2delay.eastmoney.com/api/qt/stock/kline/get" }),
]);
...
function sourceHealth(logs, now) {
  const total = logs.length;
  const successes = logs.filter((entry) => entry.ok === true);
  let consecutiveFailures = 0;
  let failureSince = null;
  for (const entry of logs) {
    if (entry.ok === true) break;
    consecutiveFailures += 1;
    failureSince = entry.time ?? failureSince;
  }
  const failureSinceMs = Number.isFinite(Date.parse(failureSince ?? "")) ? Date.parse(failureSince) : null;
  const durations = logs.map((entry) => Number(entry.durationMs)).filter(Number.isFinite);
  return {
    samples: total,
    successes: successes.length,
    failures: total - successes.length,
    successRate: total === 0 ? null : Math.round(successes.length / total * 1_000_000) / 1_000_000,
    consecutiveFailures,
    consecutiveFailureSince: failureSince,
    consecutiveFailureDurationMs: failureSinceMs === null ? 0 : Math.max(0, now - failureSinceMs),
    latencyMs: {
      p50: percentile(durations, 0.5),
      p95: percentile(durations, 0.95),
    },
    lastValidAt: successes[0]?.time ?? null,
  };
}
...
export class SourceRegistry {
  ...
  /**
   * @param opts.skipCircuitCheck run even while the breaker is open — for
   *   callers that already decided to degrade explicitly.
   */
  async trackedCall(entry, fn, opts = {}) {
    if (!opts.skipCircuitCheck && this.isCircuitOpen(entry.sourceId)) {
  ...
  dataSourceView(runtime, { staleQuoteMs = STALE_QUOTE_MS } = {}) {
    ...
    const rows = DATA_SOURCES.map((source) => {
      ...
      const circuit = this.circuitInfo(source.id);
      ...
      return {
        ...
        url: source.url,
        status: circuit.open ? "circuit_open" : last ? (last.ok ? "ok" : "error") : "unknown",
        lastTime: last?.time,
        lastOk: last?.ok,
        ...
        circuit,
        health: sourceHealth(logs, now),
      };
    });
    for (const row of rows) {
      const peers = rows.filter((candidate) => candidate.id !== row.id
        && candidate.kind === row.kind
        && candidate.instrument === row.instrument);
      row.health.availableFallbackSources = peers
        .filter((candidate) => candidate.status === "ok" && candidate.circuit.open !== true)
        .map((candidate) => candidate.id);
      row.health.availableFallbackCount = row.health.availableFallbackSources.length;
      if (row.id === "cmb-market-center" && runtime.historicalStatus) {
        row.health.executionHistory = {
          records: runtime.historicalStatus.records,
          realRatio: runtime.historicalStatus.realRatio,
          syntheticRatio: runtime.historicalStatus.syntheticRatio,
          proxyRatio: runtime.historicalStatus.proxyRatio,
          unknownRatio: runtime.historicalStatus.unknownRatio,
          lastValidAt: runtime.historicalStatus.lastValidAt,
          gaps: runtime.historicalStatus.gaps,
        };
      }
    }
    return rows;
  ...
  /**
   * Fetch Eastmoney klines. `endDate` ("YYYYMMDD") pins the response to
   * klines up to that Beijing calendar date — plan-06 replay statistics use
   * it for point-in-time (no-lookahead) per-day pulls; the default sentinel
   * keeps the historical "up to now" behaviour.
   *
   * Host fallback: the primary host intermittently answers with empty replies
   * or drops connections (anti-scraping, observed in production); the delayed
   * mirror then serves identical closed-history rows, so callers get their
   * bars instead of a failed lane.
   */
  async fetchEastmoneyBars(secid, klt, limit = 120, endDate = "20500101") {
    const sourceId = secid === "118.AU9999" ? "eastmoney-kline-au" : secid === "122.XAU" ? "eastmoney-kline-xau" : "eastmoney-kline-au";
    // The delayed mirror keeps its OWN breaker: a blocked primary must be
    // able to degrade to the mirror, and a failing mirror must not extend
    // the primary's outage bookkeeping (live polling stays protected).
    const mirrorSourceId = `${sourceId}-delayed`;
    const attempt = (host, id) => {
      const url = eastmoneyKlineUrl(host, secid, klt, limit, endDate);
      return this.trackedCall({
        sourceId: id,
        source: "eastmoney",
        kind: "history",
    ...
    let lastError = null;
    try {
      const bars = await attempt("push2his.eastmoney.com", sourceId);
    ...
    try {
      const bars = await attempt("push2delay.eastmoney.com", mirrorSourceId);
      if (Array.isArray(bars) && bars.length > 0) return bars;
      lastError = new Error("empty klines");
    } catch (error) {
      lastError = error;
    }
    throw lastError ?? new Error("eastmoney kline fetch failed");
  }
}
...
export const fetchEastmoneyBars = (secid, klt, limit = 120, endDate = "20500101") => getDefaultSourceRegistry().fetchEastmoneyBars(secid, klt, limit, endDate);
