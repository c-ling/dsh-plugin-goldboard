...
// plan-04: user-settings seam. The host registers its config namespace with
// ctx.settings when a provider is mounted and falls back to classic
// storages/<plugin>/config.json otherwise (installSettingsSection is a no-op
// without a provider). Dependencies are pinned to the running Harness line
// (0.1.1-rc.x) — see docs/plans/plan-04-settings-migration.md.
...
// ── plan-03: signal-lane stickiness / dynamic CMB spread / premium stats ───
...
const LANE_SWITCH_TICKS = 3;
const LANE_RECOVER_TICKS = 3;
...
const SIGNAL_LANES = Object.freeze(["CMB", "XAU", "AU9999"]);
...
const CMB_SPREAD_SAMPLE_CAPACITY = 512;
const CMB_SPREAD_SAMPLE_TTL_MS = 6 * 60 * 60 * 1000;
const CMB_SPREAD_MIN_SAMPLES = 30;
const CMB_SPREAD_MIN_INTERVAL_MS = 60_000;
...
const PREMIUM_HISTORY_CAP = 60;
const PREMIUM_MIN_DAYS = 20;
// Intraday premium observations kept for one Beijing day's running median.
const PREMIUM_DAY_SAMPLE_CAP = 1440;
...
// ── plan-03 helpers: lane resolution / weakness bar / premium σ / spread ──
...
export function computePlan(runtime, config, now = new Date()) {
  ...
  // Dynamic CMB spread calibration (plan-03 03.3): with enough fresh live-CMB
  // samples the fallback estimate uses their median instead of the static
  // configured spread; both sides share the mid-price-calibrated offset.
  const dynamicSpread = dynamicCmbSpread(runtime.cmbSpreadSamples, now.getTime(), cfg.cmb.sellSpreadPerGram);
  const cmbBuyOffset = dynamicSpread ? dynamicSpread.spread : cfg.cmb.buySpreadPerGram;
  const cmbSellOffset = dynamicSpread ? dynamicSpread.spread : cfg.cmb.sellSpreadPerGram;
  ...
  function finish(plan) {
    ...
    if (laneDecision.pendingLane !== null && laneDecision.pendingTicks > 0 && !plan.reasonCodes.includes("signal_lane_degraded")) {
  ...
  // Pending-downgrade window: the established lane's own quote may already be
  // gone (e.g. XAU lane, quote wiped) while the switch is still counting.
  // Fall through to the next available pricing source instead of producing
  // null prices; the degraded marker on the plan explains the state.
  const price = liveCmb
  ...
  const cmbBuy = liveCmb ? round2(Number(cmbQuote.buyPrice)) : round2(cmbBase + cmbBuyOffset);
  const cmbSell = liveCmb ? round2(Number(cmbQuote.sellPrice)) : round2(cmbBase + cmbSellOffset);
  ...
  if (pos.grams > 0) {
    ...
    if (cmbNow >= targetPrice) {
      ...
      if (overboughtBearish) {
        ...
        const lastClosed5m = bars5[bars5.length - 1];
        const prevClosed5m = bars5[bars5.length - 2];
        const weaknessBar = isBearishWeaknessBar(lastClosed5m, prevClosed5m, ind5.atr14, {
          shadowAtrMult: strategy.weaknessShadowAtrMult,
        });
        const weaknessExit = Number.isFinite(ind5.rsi14) && ind5.rsi14 > strategy.weaknessRsi && weaknessBar.weakness;
        if (weaknessExit) {
          const reduceGrams = stageReduceGrams(cfg.limits.maxGrams, pos.grams, "sell_weakness", cfg.strategy.minRemainGrams);
          if (reduceGrams > 0) {
...
export async function dispatchAlert(config, message, logger) {
  ...
  if (wh.feishu.enabled) jobs.push(["feishu", () => sendFeishu(wh.feishu, renderWebhookTemplate(wh.feishu.bodyTemplate, message))]);
  if (wh.dingtalk.enabled) jobs.push(["dingtalk", () => sendDingtalk(wh.dingtalk, renderWebhookTemplate(wh.dingtalk.bodyTemplate, message))]);
  if (wh.wecom.enabled) jobs.push(["wecom", () => sendWecom(wh.wecom, renderWebhookTemplate(wh.wecom.bodyTemplate, message))]);
  wh.generic.forEach((generic, index) => {
    if (generic.enabled) {
      const channel = generic.id ? `generic:${generic.id}` : `generic:${index + 1}`;
      jobs.push([channel, () => sendGeneric(generic, renderWebhookTemplate(generic.bodyTemplate, message))]);
    }
  });
...
/**
 * Outer safety net for every route handler (plan-03 03.4): the harness
 * webServer answers a handler exception with a bare 400 and no JSON body,
 * which violates our own DESIGN §8 envelope contract. POST routes keep their
 * inner error mapping (409/413/422/…); anything that escapes it still gets a
 * well-formed 500 envelope here instead of the harness's bare 400. The
 * message is a sanitized summary — no stack trace.
 */
...
export function apply(ctx, config = {}) {
  ...
  //
  // Two cooperating wirings:
  //
  // 1. `installSettingsSection` (from @deepseek-ai/dsh-settings) registers the
  //    namespace while a settings provider is mounted, layering schema
  //    defaults → entry config (base) → user document, and points `configSource`
  //    at the resolved scope. Without a provider it never runs and the plugin
  //    keeps the classic storages/…/config.json path unchanged.
  // 2. Our own `ctx.inject(["settings"], …)` captures the service reference so
  //    the legacy-config migration and the POST /config write-through can reach
  //    it. cordis only lets fibers that declared the dependency read service
  //    properties — direct `ctx.settings` access would throw.
  //
  // `usingSettings` is a dynamic flag set once at provider mount (never probed
  // per request); route handlers branch on it.
  ...
  function migrateLegacyConfig() {
    ...
    migrationPromise = (async () => {
      let legacy = null;
      try {
        legacy = await readJson(configPath, null);
      } catch {
        legacy = null;
      }
      ...
      if (!hasUserSection) {
        await settingsRef.replace(SETTINGS_NAMESPACE, normalizeConfig({ ...DEFAULT_CONFIG, ...legacy }));
  ...
  // of the namespace (plan-04 04.5).
  const entrySections = normalizeConfig(config);
  ...
  async function logAlert(message, channelResults) {
    runtime.lastAlertLog.unshift({
      ...
      // plan-03 03.6: dispatchAlert now reports per-channel outcomes, so
      // sentTo records where the notification actually went.
  ...
  async function evaluateAlerts() {
    ...
    if (plan.laneSwitchedFrom && plan.signalLane && plan.signalLane !== plan.laneSwitchedFrom) {
      ...
      if (priorityTo > priorityFrom) {
        ...
        await logAlert(message, await dispatchAlert(runtime.config, message, logger));
        return;
      }
    }
  ...
  if (webServer) {
          ...
          if (req.method === "POST") {
            try {
              ...
              if (usingSettings && settingsRef && settingsWritable) {
                // plan-04 04.3: provider environments persist through the
                // settings service (settings.yaml); config.json is retired.
                // The route stays as the write path for clients without a
                // writable settingsScope (remote browsers — the settings RPCs
                // are loopback-only) and for the fallback branch below.
                await settingsRef.replace(SETTINGS_NAMESPACE, nextConfig);
                // Adopt immediately: the commit's watch → onChange lands
                // asynchronously and would reconcile to the same value.
    ...
    for (const route of routes) {
      ctx.effect(
        () => webServer.register({ kind: "exact", path: route.path, handler: safeRouteHandler(route.handler) }),
        `dsh-plugin-goldboard: route ${route.path}`,
      );
    }
  }
