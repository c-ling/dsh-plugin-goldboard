...
function seedBars(bars, klines, metadata = {}) {
  mergeKlines(bars[5], klines, 5, metadata);
  mergeKlines(bars[60], klines, 60, metadata);
}
...
export function listMissingCmbMinuteSlots(runtime, config, now = new Date()) {
  ...
  for (let t = midnight; t <= end; t += 60_000) {
    if (!isOpenMinute(config, t)) continue;
 ...
 */
export function isOpenMinute(config, timestamp) {
  ...
  const todayTradeable = isTradingDate(cfg, parts.date, parts.day);
  ...
  const todayClose = Math.min(closeMin, 1440);
  if (todayTradeable && parts.minutes >= openMin && parts.minutes < todayClose) return true;
  if (yesterdayTradeable && closeMin > 1440 && parts.minutes < closeMin - 1440) return true;
  return false;
}
...
export function filterBarsToTradingHours(bars, config) {
  ...
  const cfg = normalizeConfig(config);
  return bars.filter((bar) => bar && Number.isFinite(bar.t) && isOpenMinute(cfg, bar.t));
}
...
export function windowCoverage(bars1m, now, minutes, config) {
  ...
  if (config) {
    ...
    while (slots.length < minutes && cursor > scanEnd) {
      if (isOpenMinute(cfg, cursor)) slots.push(cursor);
      cursor -= 60_000;
    }
...
export function computePlan(runtime, config, now = new Date()) {
  ...
  // Prefer checking all 5/10/30/60 windows whenever the session-aware window
  // already has enough 30/60-minute data (e.g. 09:xx can reuse the previous
  // session's pre-02:00 data). Only fall back to the 5/10-only warm-up rule
  // when those longer windows are still genuinely thin.
  ...
  const gate = coverageGate(gateBars1m, now, gateWindows, cfg);
...
function barsView(bars, limit = 288, intervalMinutes = 1, now = Date.now()) {
  if (!Array.isArray(bars)) return [];
...
export function buildSnapshot(runtime, config, now = new Date()) {
  ...
  return {
    ...
    quality,
    trend: {
      AU9999_1m: barsView(filterBarsToTradingHours(runtime.bars?.AU9999?.[1], cfg), 1440, 1, now),
      XAU_1m: barsView(filterBarsToTradingHours(runtime.bars?.XAU?.[1], cfg), 1440, 1, now),
      GCF_1m: barsView(filterBarsToTradingHours(runtime.bars?.GCF?.[1], cfg), 1440, 1, now),
      CMB_1m: barsView(filterBarsToTradingHours(runtime.bars?.CMB?.[1], cfg), 1440, 1, now),
    },
    indicators: plan.indicators ?? {},
  ...
  async function seedHistory() {
    ...
    if (au5.length < 60) {
      try {
        ...
        if (klines.length > 0) seedBars(runtime.bars.AU9999, filterBarsToTradingHours(klines, runtime.config), AU_BAR_META);
      } catch (error) {
    ...
    if (runtime.bars.AU9999[60].length < 20) {
      try {
        ...
        if (klines.length > 0) mergeKlines(runtime.bars.AU9999[60], filterBarsToTradingHours(klines, runtime.config), 60, AU_BAR_META);
      } catch (error) {
    ...
    if (xau5.length < 60) {
      try {
        ...
        if (klines.length > 0) seedBars(runtime.bars.XAU, filterBarsToTradingHours(klines, runtime.config), XAU_BAR_META);
      } catch (error) {
    ...
    if (runtime.bars.XAU[60].length < 20) {
      try {
        ...
        if (klines.length > 0) mergeKlines(runtime.bars.XAU[60], filterBarsToTradingHours(klines, runtime.config), 60, XAU_BAR_META);
      } catch (error) {
    ...
    if (auDaily.length < 20) {
      try {
        ...
        if (klines.length > 0) mergeKlines(runtime.bars.AU9999[1440], klines, 1440, AU_BAR_META);
      } catch (error) {
      ...
      if ((runtime.bars.AU9999[1440]?.length ?? 0) < 20) {
        try {
          ...
          if (daily.length > 0) mergeKlines(runtime.bars.AU9999[1440], daily, 1440, { ...AU_BAR_META, source: "sge" });
        } catch (sgeError) {
    ...
    if (xauDaily.length < 20) {
      try {
        ...
        if (klines.length > 0) mergeKlines(runtime.bars.XAU[1440], klines, 1440, XAU_BAR_META);
      } catch (error) {
    ...
    if (gcfDaily.length < 20) {
      try {
        ...
        if (daily.length > 0) mergeKlines(runtime.bars.GCF[1440], daily, 1440, GCF_BAR_META);
      } catch (yahooError) {
  ...
  function backfillCmbFromInternational() {
    ...
    for (const interval of [1, 5]) {
      ...
      for (const bar of xauBars) {
        ...
        if (!isOpenMinute(runtime.config, bar.t)) continue;
