...
export function computePlan(runtime, config, now = new Date()) {
  ...
  // hour after the session opens, only 5/10-minute windows are checked (the
  // 30/60-minute windows are naturally thin right after the open); the data
  // referenced by the plan still covers 5/10/30/60 as far as possible.
  ...
  const gateWindows = Number.isFinite(minutesSinceOpen) && minutesSinceOpen * 60_000 < SESSION_WARMUP_MS ? [5, 10] : PLAN_WINDOWS;
