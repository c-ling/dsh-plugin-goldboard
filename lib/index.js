...
export function computePlan(runtime, config, now = new Date()) {
  ...
  const longWindowsReady = fullGate.coverage[30] > MIN_WINDOW_COVERAGE && fullGate.coverage[60] > MIN_WINDOW_COVERAGE;
...
export function buildSnapshot(runtime, config, now = new Date()) {
  ...
  return {
    ...
    derived,
    trend: {
      AU9999_1m: barsView(filterBarsToTradingHours(runtime.bars?.AU9999?.[1], cfg), 1440),
      XAU_1m: barsView(filterBarsToTradingHours(runtime.bars?.XAU?.[1], cfg), 1440),
      CMB_1m: barsView(filterBarsToTradingHours(runtime.bars?.CMB?.[1], cfg), 1440),
    },
...
export function apply(ctx, config = {}) {
  ...
  function recordQuote(key, quote) {
    ...
    if (computeMarketState(runtime.config, new Date(quote.updatedAt)).state === "open") {
  ...
  async function seedHistory() {
    ...
    if (au5.length < 60) {
      try {
        ...
        if (klines.length > 0) seedBars(runtime.bars.AU9999, filterBarsToTradingHours(klines, runtime.config));
      } catch (error) {
    ...
    if (runtime.bars.AU9999[60].length < 20) {
      try {
        ...
        if (klines.length > 0) mergeKlines(runtime.bars.AU9999[60], filterBarsToTradingHours(klines, runtime.config), 60);
      } catch (error) {
    ...
    if (xau5.length < 60) {
      try {
        ...
        if (klines.length > 0) seedBars(runtime.bars.XAU, filterBarsToTradingHours(klines, runtime.config));
      } catch (error) {
    ...
    if (runtime.bars.XAU[60].length < 20) {
      try {
        ...
        if (klines.length > 0) mergeKlines(runtime.bars.XAU[60], filterBarsToTradingHours(klines, runtime.config), 60);
      } catch (error) {
  ...
  async function backfillTrend() {
    ...
    if (auMissing) {
      try {
        const klines = await fetchEastmoneyBars("118.AU9999", 5, 288);
        if (klines.length > 0) mergeKlines(runtime.bars.AU9999[5], filterBarsToTradingHours(klines, runtime.config), 5);
      } catch (error) {
    ...
    if (xauMissing) {
      try {
        const klines = await fetchEastmoneyBars("122.XAU", 5, 288);
        if (klines.length > 0) mergeKlines(runtime.bars.XAU[5], filterBarsToTradingHours(klines, runtime.config), 5);
      } catch (error) {
      ...
      },
      {
        ...
        handler: async (req, res) => {
          ...
          sendJson(res, 200, {
            ...
            instrument,
            interval,
            bars: barsView(filteredBars, limit),
          });
        },
      },
      {
