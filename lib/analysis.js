...
export function buildAnalysisInput(context, selection, config) {
  ...
  return {
    ...
    position: {
      ...
      feeAdjustedPnl: snapshot.position?.feeAdjustedPnl ?? 0,
    },
    plan: {
      action: snapshot.plan?.action ?? "no_data",
      reasonCodes: snapshot.plan?.reasonCodes ?? [],
      dataCoverage: snapshot.plan?.dataCoverage ?? {},
      confidenceScore: snapshot.plan?.confidenceScore,
      confidenceMax: snapshot.plan?.confidenceMax,
...
export function validateAnalysisOutput(value, context) {
  ...
  const allowedKeys = new Set(["status", "bias", "horizon", "action", "confidence", "evidence", "support", "resistance", "reassessWhen", "invalidations", "dataWarnings", "suggestedOrder", "riskDisclosure"]);
...
function qualityCode(quality) {
  const reasons = new Set(quality?.reasonCodes ?? []);
  if (reasons.has("data_stale")) return "DATA_STALE";
  if (reasons.has("indicator_warmup")) return "WARMUP_NOT_READY";
  if (reasons.has("instrument_mismatch")) return "INSTRUMENT_MISMATCH";
...
export class AnalysisModule {
  ...
  async run(request = {}) {
    ...
    if (context.snapshot.quality?.ready !== true) {
      return {
        ok: false,
        status: "blocked",
        error: { code: qualityCode(context.snapshot.quality) },
        quality: context.snapshot.quality,
        snapshotAsOf: context.snapshot.serverTime,
      };
    }
    let selection;
    try {
