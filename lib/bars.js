...
export const CMB_BAR_META = Object.freeze({
  source: "manual",
  synthetic: true,
  quality: "proxy",
  executionEvidence: Object.freeze({ mode: "proxy", askSource: "manual", bidSource: null }),
...
function upsertBar(list, bar) {
  ...
  if (last && last.t === bar.t) {
    ...
    for (const key of [
      "dataSchemaVersion", "source", "sourceTimestamp", "receivedAt", "ingestedAt",
      "sourceDelayMs", "futureSkewMs", "quality", "instrument", "market", "currency", "unit",
    ]) {
    ...
    if (compatibleSides) {
      ...
      last.customerAsk = last.askC;
      last.customerBid = last.bidC;
      last.customerAskSource = bar.customerAskSource;
      last.customerBidSource = bar.customerBidSource;
      last.executionEvidence = bar.executionEvidence;
    } else {
      last.executionSideComplete = false;
      last.executionEvidence = {
        mode: last.executionEvidence?.mode === "proxy" || bar.executionEvidence?.mode === "proxy" ? "proxy" : "unknown",
        askSource: null,
        bidSource: null,
      };
      last.customerAsk = null;
      last.customerBid = null;
      last.customerAskSource = null;
      last.customerBidSource = null;
...
export function aggregateSubBars(list, subKlines, intervalMinutes, metadata = {}, now = new Date()) {
  ...
  const normalizedSubs = (Array.isArray(subKlines) ? subKlines : [])
    .map((raw) => normalizeBarRecord(raw, { ...metadata, synthetic: false }))
    .filter(Boolean)
    .sort((a, b) => a.t - b.t);
  const positiveDiffs = [];
  for (let index = 1; index < normalizedSubs.length; index += 1) {
    const diff = normalizedSubs[index].t - normalizedSubs[index - 1].t;
    if (diff > 0 && diff <= spanMs) positiveDiffs.push(diff);
  }
  const childSpanMs = Number.isFinite(Number(metadata.childIntervalMinutes))
    ? Number(metadata.childIntervalMinutes) * 60_000
    : positiveDiffs.length > 0 ? Math.min(...positiveDiffs) : 5 * 60_000;
  const expectedSamples = Math.max(1, Math.round(spanMs / childSpanMs));
  const buckets = new Map();
  for (const normalized of normalizedSubs) {
    const t = alignStart(normalized.t, intervalMinutes);
    if (t + spanMs > nowMs) continue;
    if (!buckets.has(t)) buckets.set(t, []);
    buckets.get(t).push(normalized);
  }
  for (const [t, chunk] of buckets) {
    const first = chunk[0];
    const last = chunk[chunk.length - 1];
    const expectedTimes = new Set(Array.from({ length: expectedSamples }, (_, index) => t + index * childSpanMs));
    const actualTimes = new Set(chunk.map((bar) => bar.t));
    const partial = actualTimes.size !== expectedSamples
      || [...expectedTimes].some((value) => !actualTimes.has(value))
    ...
    const raw = {
      ...
      c: last.c,
      source: chunk.every((bar) => bar.source === first.source) ? first.source : "aggregated",
      ...
      partial,
      sampleCount: actualTimes.size,
      expectedSamples,
    };
    const sideComplete = !partial
      && chunk.every((bar) => bar.executionSideComplete === true)
      && chunk.every((bar) => bar.executionSideSource === first.executionSideSource);
    if (sideComplete) {
      raw.executionSideComplete = true;
      raw.executionSideSource = first.executionSideSource;
      raw.executionEvidence = first.executionEvidence;
      for (const prefix of ["ask", "bid"]) {
        raw[`${prefix}O`] = first[`${prefix}O`];
        raw[`${prefix}H`] = Math.max(...chunk.map((bar) => bar[`${prefix}H`]));
        raw[`${prefix}L`] = Math.min(...chunk.map((bar) => bar[`${prefix}L`]));
        raw[`${prefix}C`] = last[`${prefix}C`];
      }
    } else {
      raw.executionSideComplete = false;
    ...
    const normalized = normalizeBarRecord(raw, metadata);
    if (normalized) byTime.set(t, normalized);
  }
...
export function seedBars(bars, klines, metadata = {}, now = new Date()) {
  const nowMs = new Date(now).getTime();
  const closedSubBars = (Array.isArray(klines) ? klines : [])
    .filter((bar) => Number(bar?.t) + 5 * 60_000 <= nowMs);
  mergeKlines(bars[5], closedSubBars, 5, metadata);
  aggregateSubBars(bars[60], closedSubBars, 60, metadata, now);
}
  ...
  if (added > 0) {
    ...
    for (const interval of [5, 15, 60, 1440]) {
      ...
      for (const [bucket, bars] of groups) {
        const uniqueMinutes = new Set(bars.map((item) => alignStart(item.t, 1)));
        const expectedSamples = interval;
        const bar = normalizeBarRecord(
          {
            t: bucket,
            o: bars[0].o,
            ...
            ...CMB_BAR_META,
            partial: uniqueMinutes.size !== expectedSamples,
            sampleCount: uniqueMinutes.size,
            expectedSamples,
