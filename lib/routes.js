...
export function createRoutes(deps) {
  ...
  return [
    ...
    {
      // plan-06 batch replay statistics. POST { days?, force?, detail? }
    ...
    },
    {
      ...
      handler: route({
        GET: async (req, res) => {
          sendJson(res, 200, { ok: true, sources: registry.dataSourceView(runtime) });
        },
      }),
    },
    {
        ...
        POST: async (req, res) => {
          ...
          try {
            ...
            if (channel === "system") {
              ...
              await sendGeneric(testConfig, renderWebhookTemplate(testConfig.bodyTemplate, testMessage), deps.post);
            } else {
