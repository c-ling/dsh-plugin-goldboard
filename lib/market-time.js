 ...
 * can reuse data from the previous session before 02:00. A contiguous run of
 * ≥ COVERAGE_REANCHOR_GAP_MINUTES missing trading minutes is treated as an
 * outage seam and excluded (see the comment inside).
 */
...
export function windowCoverage(bars1m, now, minutes, config) {
  ...
  if (config) {
    ...
    // Outage-seam re-anchor: when scanning backward, a contiguous run of ≥8
    // missing trading minutes is an outage seam (host sleep / source outage).
    // Everything older belongs to the previous uninterrupted segment, so the
    // seam tail is dropped from the denominator instead of poisoning the
    // window until the hole ages out of it. Observed 2026-08-25: after data
    // resumed at 10:34 following a 55-minute sleep hole, the 60m gate stayed
    // red for another full hour — with the seam anchor it reopens immediately.
    // Short windows (< the seam length) never complete the run and keep the
    // plain rolling behaviour.
    const slots = [];
    let missingRun = 0;
    let cursor = end;
    ...
    while (slots.length < minutes && cursor > scanEnd) {
      if (isOpenMinute(calendar, cursor)) {
        if (byMinute.has(cursor)) missingRun = 0;
        else {
          missingRun += 1;
          if (missingRun >= COVERAGE_REANCHOR_GAP_MINUTES) {
            // Drop the seam tail (the missing slots already pushed; the
            // current one was not pushed) and stop — older slots belong to
            // the previous uninterrupted segment.
            const missingPushed = missingRun - 1;
            slots.splice(slots.length - missingPushed, missingPushed);
            break;
          }
        }
        slots.push(cursor);
