...
export function buildAlertMessage(plan, config, localeHint = "zh") {
  ...
  if (isBuy) {
    // Header "招行买入价" should be the current executable ask, not the
    // suggested limit order price stored in order.cmbEstimatedPrice.
    const executionAsk = Number(plan?.execution?.ask);
    ...
    if (Number.isFinite(executionAsk) && executionAsk > 0) {
      cmbPrice = round2(executionAsk);
    } else if (Number.isFinite(buyBase)) {
    ...
    // The order must display the executable quote side. Fees and slippage are
    // separate ledger components; subtracting them here mislabels net proceeds
    // as the bank's quoted sell price.
    cmbPrice = rawCmbPrice;
    ...
    if (isSell) {
      const sellQuote = round2(fallbackBase + cfg.cmb.sellSpreadPerGram);
      if (Number.isFinite(sellQuote)) cmbPrice = sellQuote;
    } else {
...
async function postJson(url, value, post = defaultPost, headers = {}) {
  ...
  if (response?.ok === false || Number.isFinite(status) && (status < 200 || status >= 300)) {
    throw Object.assign(new Error(`webhook HTTP ${Number.isFinite(status) ? status : "error"}`), {
...
function blockedWebhook(message) {
  return Object.assign(new Error(message), { statusCode: 400, code: "WEBHOOK_URL_BLOCKED" });
}
...
function privateIp(address) {
  ...
  if (kind === 4) return privateIpv4(value);
...
function safeGenericHeaders(value) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const out = {};
  for (const [rawName, rawValue] of Object.entries(source)) {
    const name = String(rawName).toLowerCase();
    const headerValue = String(rawValue);
    const allowed = name === "authorization" || name === "accept" || name.startsWith("x-");
    if (!allowed || /[\r\n]/.test(name) || /[\r\n]/.test(headerValue)) {
      throw blockedWebhook(`generic webhook header is not allowed: ${rawName}`);
    }
...
/** Validate the generic outbound target at the single Host-side transport seam. */
export async function validateGenericWebhookConfig(cfg, options = {}) {
  if (!cfg?.url) throw webhookConfigError("通用 webhook 地址未配置");
  let url;
  try {
    url = new URL(cfg.url);
  } catch {
    throw blockedWebhook("generic webhook URL is invalid");
  }
  if (url.protocol !== "https:" || url.username || url.password) {
    throw blockedWebhook("generic webhook URL must be credential-free HTTPS");
  }
  const hostname = url.hostname.toLowerCase().replace(/\.$/, "").replace(/^\[|\]$/g, "");
  if (hostname === "localhost" || hostname.endsWith(".localhost") || hostname.endsWith(".local")) {
    throw blockedWebhook("generic webhook hostname is local");
  }
  let addresses;
  if (isIP(hostname)) {
    if (privateIp(hostname)) throw blockedWebhook("generic webhook address is not public");
    addresses = [{ address: hostname, family: isIP(hostname) }];
  } else {
    try {
      addresses = await (options.lookup ?? lookup)(hostname, { all: true, verbatim: true });
    } catch {
      throw blockedWebhook("generic webhook hostname could not be resolved");
    }
    if (!Array.isArray(addresses) || addresses.length === 0 || addresses.some((entry) => privateIp(entry?.address))) {
      throw blockedWebhook("generic webhook hostname resolves to a non-public address");
    }
  }
  ...
  return {
    ...
    headers: safeGenericHeaders(cfg.headers),
  };
}

/**
 * HTTPS POST pinned to the DNS answer already validated above. TLS still
 * verifies the original hostname through SNI, and Host remains the URL host.
 */
function postValidatedGeneric(validated, value, headers = {}) {
  ...
  return new Promise((resolve, reject) => {
    const request = httpsRequest({
      ...
      response.on("data", (chunk) => {
        size += chunk.length;
        if (size > 1024 * 1024) {
          request.destroy(Object.assign(new Error("webhook response too large"), { code: "WEBHOOK_RESPONSE_TOO_LARGE" }));
          return;
        }
        chunks.push(chunk);
      });
    ...
    request.on("timeout", () => request.destroy(Object.assign(new Error("webhook timeout"), { code: "WEBHOOK_TIMEOUT" })));
    request.on("error", reject);
    request.end(body);
  });
}

export async function sendGeneric(cfg, text, options = {}) {
  if (!cfg?.enabled) throw webhookConfigError("通用 webhook 未启用");
  const validated = await validateGenericWebhookConfig(cfg, { lookup: options.lookup });
  const headers = { "user-agent": USER_AGENT, ...validated.headers };
  const transport = options.transport ?? postValidatedGeneric;
  await postJson(
    validated.url,
    { text },
    (_url, value, outboundHeaders) => transport(validated, value, outboundHeaders),
    headers,
  );
}
 ...
 *
 * @param options.post optional provider-channel transport for tests/embedding.
 * @param options.genericTransport optional pinned-target generic transport.
 * @param options.genericLookup optional DNS lookup adapter used before pinning.
 */
export async function dispatchAlert(config, message, logger, options = {}) {
  ...
  wh.generic.forEach((generic, index) => {
    if (generic.enabled) {
      jobs.push([genericChannelName(generic, index), () => sendGeneric(generic, renderWebhookTemplate(generic.bodyTemplate, message), {
        lookup: options.genericLookup,
        transport: options.genericTransport,
      })]);
    }
  });
