...
export function parseQuoteTimestamp(quote, now = new Date()) {
  ...
  const raw = quote.sourceTimestamp ?? quote.timestamp ?? quote.time;
  if (raw === null || raw === undefined || raw === "") return null;

  // Tencent compact timestamps are Beijing wall-clock values. Handle them
  // before generic numeric parsing so 20260828200250 never becomes epoch ms.
  const compact = String(raw).trim();
  if (/^\d{14}$/.test(compact)) {
    const timestamp = Date.parse(
      `${compact.slice(0, 4)}-${compact.slice(4, 6)}-${compact.slice(6, 8)}`
      + `T${compact.slice(8, 10)}:${compact.slice(10, 12)}:${compact.slice(12, 14)}+08:00`,
    );
    return Number.isFinite(timestamp) ? timestamp : null;
  }

  if (typeof raw === "number" || /^\d{10,13}$/.test(compact)) {
    const numeric = Number(raw);
    const timestamp = numeric > 0 && numeric < 100_000_000_000 ? numeric * 1000 : numeric;
    return Number.isFinite(timestamp) && Number.isFinite(new Date(timestamp).getTime()) ? timestamp : null;
  }

  // Fully-qualified ISO/RFC timestamps keep their own timezone semantics.
