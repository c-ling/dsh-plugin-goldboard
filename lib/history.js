...
import { TROY_OUNCE_GRAMS, inspectXauConversion } from "./sizing.js";
import {
  AU_BAR_META,
  GCF_BAR_META,
  MAX_BARS,
...
export function createHistoryJobs({ runtime, registry, statePersister, logger }) {
  ...
  async function seedInstrumentIntraday(lane, secid, meta, label) {
    if ((runtime.bars[lane][5]?.length ?? 0) < 60) {
      try {
        ...
        if (klines.length > 0) {
          seedBars(runtime.bars[lane], filterBarsToMarketHours(klines, lane, runtime.config), meta);
    ...
    if ((runtime.bars[lane][60]?.length ?? 0) < 20) {
      try {
        ...
        if (klines.length > 0) {
          mergeKlines(runtime.bars[lane][60], filterBarsToMarketHours(klines, lane, runtime.config), 60, meta);
          statePersister.markDirty("bars");
        }
      } catch (error) {
  ...
  async function backfillLane(lane, secid, meta) {
    try {
      const klines = await registry.fetchEastmoneyBars(secid, 5, 480);
      if (klines.length > 0) mergeKlines(runtime.bars[lane][5], filterBarsToMarketHours(klines, lane, runtime.config), 5, meta);
    } catch (error) {
  ...
  function backfillCmbFromInternational(now = new Date()) {
    const conversion = inspectXauConversion({ xau: runtime.quotes.XAU, usdcny: runtime.quotes.USDCNY, asOf: now });
    const basePrice = conversion.value;
    ...
    for (const interval of [1, 5]) {
      ...
      for (const bar of xauBars) {
        ...
        const derived = normalizeBarRecord({
          t: bar.t,
          ...
          executionEvidence: { mode: "proxy", askSource: "xau-fx-derived", bidSource: null },
  ...
  /** Sample live CMB buy/sell offsets over xauCny for the dynamic fallback. */
  function sampleCmbSpread(now = new Date()) {
    ...
    samples.push({
      t: nowMs,
      buyOffset: round2(buy - base),
      sellOffset: round2(sell - base),
      // Retained as a diagnostic/migration aid; consumers use the side fields.
      spreadMid: round2((buy + sell) / 2 - base),
      sources: {
        cmb: cmb.source ?? "unknown",
        xau: runtime.quotes.XAU?.source ?? "unknown",
        usdcny: runtime.quotes.USDCNY?.source ?? "unknown",
      },
      sourceTimestamps: {
        cmb: cmb.sourceTimestamp ?? null,
        xau: runtime.quotes.XAU?.sourceTimestamp ?? null,
        usdcny: runtime.quotes.USDCNY?.sourceTimestamp ?? null,
      },
    });
