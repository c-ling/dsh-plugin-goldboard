...
function quoteView(quote, now) {
  ...
  return {
    ...
    time: quote.time,
    date: quote.date,
    dataSchemaVersion: quote.dataSchemaVersion ?? MARKET_DATA_SCHEMA_VERSION,
    source: quote.source,
    ...
    sourceDelayMs: quote.sourceDelayMs ?? null,
    futureSkewMs: quote.futureSkewMs ?? null,
    staleAfterMs: quote.staleAfterMs,
    quality: quote.quality,
    synthetic: quote.synthetic === true,
    executionSideComplete: quote.executionSideComplete === true,
    executionEvidence: quote.executionEvidence ?? { mode: "unknown", askSource: null, bidSource: null },
    customerAsk: quote.customerAsk ?? quote.customerBuy ?? quote.buyPrice ?? null,
    customerBid: quote.customerBid ?? quote.customerSell ?? quote.sellPrice ?? null,
    customerAskSource: quote.customerAskSource ?? null,
    customerBidSource: quote.customerBidSource ?? null,
...
export function barsView(bars, limit = 288, intervalMinutes = 1, now = Date.now(), meta = true) {
  ...
  return bars.slice(-limit).map((bar) => (
    meta ? {
      ...
      l: round2(bar.l),
      c: round2(bar.c),
      complete: bar.partial !== true && isBarClosed(bar, intervalMinutes, now),
      dataSchemaVersion: bar.dataSchemaVersion ?? MARKET_DATA_SCHEMA_VERSION,
      synthetic: bar.synthetic === true,
      quality: bar.quality ?? "unknown",
      sourceTimestamp: bar.sourceTimestamp ?? new Date(bar.t).toISOString(),
      receivedAt: bar.receivedAt ?? null,
      ingestedAt: bar.ingestedAt ?? null,
      ...
      executionEvidence: bar.executionEvidence ?? { mode: "unknown", askSource: null, bidSource: null },
      ...
      ...(["askO", "askH", "askL", "askC", "bidO", "bidH", "bidL", "bidC"].reduce((out, key) => {
        if (Number.isFinite(Number(bar[key]))) out[key] = round2(Number(bar[key]));
        return out;
      }, {})),
    } : {
      t: new Date(bar.t).toISOString(),
...
export function buildSnapshot(runtime, config, now = new Date()) {
  ...
  const xauIsSpot = !!xau
    ...
    && xau.instrument !== "GC=F"
    && xau.source !== "yahoo";
  const xauConversion = xauIsSpot
    ? inspectXauConversion({ xau, usdcny, asOf: now })
    : { ready: false, value: null, reasonCodes: ["xau_spot_unavailable"], dependencies: [] };
  const xauCny = xauConversion.value;
  const hasXauFallback = xauConversion.ready;
  ...
  const plan = basePlan.action === "no_data" && fallbackMissing
    ...
    : basePlan;
  const derived = {};
  derived.xauConversion = xauConversion;
  if (xauCny !== null) {
  ...
  if (liveCmb || hasXauFallback || hasDomestic) {
    ...
    const dynamicSpreadView = dynamicCmbSpread(runtime.cmbSpreadSamples, now.getTime(), {
      buyOffset: cfg.cmb.buySpreadPerGram,
      sellOffset: cfg.cmb.sellSpreadPerGram,
    });
    const buyOffsetView = dynamicSpreadView ? dynamicSpreadView.buyOffset : cfg.cmb.buySpreadPerGram;
    const sellOffsetView = dynamicSpreadView ? dynamicSpreadView.sellOffset : cfg.cmb.sellSpreadPerGram;
    executionQuote = resolveExecutionQuote({
      quote: liveCmb ? cmbQuote : null,
      referencePrice: cmbBase,
      buyOffset: buyOffsetView,
      sellOffset: sellOffsetView,
      source: liveCmb ? "cmb" : (xauCny !== null ? "xau-fx-fallback" : "au9999-fallback"),
      asOf: now.toISOString(),
    }, cfg);
    const cmbBuy = executionQuote.ask;
    const cmbSell = executionQuote.bid;
    const sellLedger = accountExecution({ side: "sell", grams: 1, quote: executionQuote }, cfg);
    const effectiveExitPrice = sellLedger.ok ? sellLedger.effectivePrice : null;
    derived.cmb = {
      buyPrice: cmbBuy,
      sellPrice: cmbSell,
      customerBuy: cmbBuy,
      customerSell: cmbSell,
      spread: executionQuote.spreadPerGram,
      effectiveExitPrice,
      average: round2((cmbBuy + cmbSell) / 2),
      sellPriceAfterFee: effectiveExitPrice,
      live: Boolean(liveCmb),
      synthetic: executionQuote.synthetic === true,
      quality: executionQuote.quality,
      executionVersion: executionQuote.executionVersion,
      costComponents: sellLedger.ok ? sellLedger.components : null,
      spreadSource: liveCmb
        ? "live"
        : dynamicSpreadView
          ? (dynamicSpreadView.legacy ? "dynamic-legacy" : "dynamic-bid-ask")
          : "static",
      ...
      ...(!liveCmb ? {
        basePrice: cmbBase,
        calibration: dynamicSpreadView?.calibration ?? {
          ...describeCmbSpreadSamples(runtime.cmbSpreadSamples, now.getTime()),
          method: "static-config",
        },
      } : {}),
      sourceNote: liveCmb ? "cmb-live-bid-ask" : "cmb-synthetic-bid-ask",
    };
  }
  ...
  const calendarStates = {
    CMB: computeInstrumentMarketState("CMB", cfg, now),
    AU9999: computeInstrumentMarketState("AU9999", cfg, now),
    XAU: computeInstrumentMarketState("XAU", cfg, now),
  };
  ...
  const quality = plan.dataQuality ?? assessMarketQuality({
    ...
    coverageDetails: plan.dataCoverageDetails ?? {},
  ...
  const qualityOut = !premiumStats.ready && Number.isFinite(derived.domesticPremiumPerGram)
    ...
    : quality;
  const executionFresh = liveCmb
    ? cmbView !== null && cmbView.stale !== true
    : xauCny !== null
      ? xauView !== null && xauView.stale !== true && quoteView(usdcny, now.getTime())?.stale !== true
      : auView !== null && auView.stale !== true;
  const valuationQuote = executionFresh ? executionQuote : null;
  const fallbackValuation = valuePosition(cfg.position, valuationQuote, cfg);
  const fallbackPosition = {
    grams: cfg.position.grams,
    avgCostPerGram: cfg.position.avgCostPerGram,
    effectiveEntryPrice: fallbackValuation.effectiveEntryPrice,
    cmbNow: fallbackValuation.bid,
    effectiveExitPrice: fallbackValuation.effectiveExitPrice,
    feeAdjustedPnl: fallbackValuation.pnlCny,
    profitPerGram: fallbackValuation.effectivePnlPerGram,
    exitNeeded: fallbackValuation.breakEvenBid,
    valuationAvailable: fallbackValuation.available,
    valuationReasonCode: fallbackValuation.reasonCode,
    synthetic: fallbackValuation.synthetic === true,
    costs: fallbackValuation.costs ?? null,
    lots: (cfg.position.lots ?? []).map((lot) => {
      const lotValuation = valuePosition({ grams: lot.grams, avgCostPerGram: lot.price }, valuationQuote, cfg);
      return {
        ...lot,
        effectiveEntryPrice: lotValuation.effectiveEntryPrice,
        cmbNow: lotValuation.bid,
        effectiveExitPrice: lotValuation.effectiveExitPrice,
        feeAdjustedPnl: lotValuation.pnlCny,
        profitPerGram: lotValuation.effectivePnlPerGram,
        valuationAvailable: lotValuation.available,
        valuationReasonCode: lotValuation.reasonCode,
      };
    }),
  };
  return {
    ...
    market: {
      ...
      calendarVersion: calendarStates.CMB.calendarVersion,
    },
    calendars: calendarStates,
    ...
    quality: qualityOut,
    trend: {
      AU9999_1m: barsView(filterBarsToMarketHours(runtime.bars?.AU9999?.[1], "AU9999", cfg), TREND_POINTS, 1, now, false),
      XAU_1m: barsView(filterBarsToMarketHours(runtime.bars?.XAU?.[1], "XAU", cfg), TREND_POINTS, 1, now, false),
      GCF_1m: barsView(filterBarsToMarketHours(runtime.bars?.GCF?.[1], "GCF", cfg), TREND_POINTS, 1, now, false),
      CMB_1m: barsView(filterBarsToMarketHours(runtime.bars?.CMB?.[1], "CMB", cfg), TREND_POINTS, 1, now, false),
    },
    indicators: plan.indicators ?? {},
    position: plan.position ?? fallbackPosition,
    plan: {
      action: plan.action,
      ...
      execution: plan.execution ?? executionQuote,
      reasonCodes: plan.reasonCodes ?? [],
      dataCoverage: plan.dataCoverage,
      dataCoverageDetails: plan.dataCoverageDetails,
      dataQuality: plan.dataQuality ?? quality,
      ruleScore: plan.ruleScore ?? plan.confidenceScore,
      ruleScoreMax: plan.ruleScoreMax ?? plan.confidenceMax,
