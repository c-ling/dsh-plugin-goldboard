...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var DICT = {
      zh: {
        ...
        statsTitle: "策略统计",
        statsHint: "对最近 N 个交易日按当前策略参数做批量回放统计（只读分析，不发提醒、不调模型）：默认以招行积存金信号道、基于本机积累的积存金分钟数据逐 5 分钟重建指标与规则，口径与实盘一致；统计各动作触发次数与事后命中情况；confirmBars/冷却照常生效。同参数结果缓存 1 小时；回放深度受本地积存金数据积累限制，不足的天数会如实跳过。",
        statsDays: "统计天数",
        statsGenerate: "生成统计",
        ...
        statsTableMfe30: "MFE30m 均值",
        statsTableMae30: "MAE30m 均值",
        statsTableSessionEnd: "时段末净益均值",
      ...
      en: {
        ...
        statsHint: "Batch replay of the last N trading days under the current strategy parameters (read-only analysis — no alerts, no model calls): by default indicators and rules are rebuilt bar-by-bar on the CMB accumulated-gold lane using this host's locally accumulated minute series, so the terms match the desk you actually trade; the report shows how often each action fired and how it played out afterwards; confirmBars/cooldown stay active. Same-parameter results are cached for 1 hour; depth is bounded by locally persisted data and short days are skipped honestly.",
        statsDays: "Days",
        ...
        statsEvents: "{count} signal events",
        ...
        statsTableCount: "Samples",
        statsTableTargetHit: "Target hit rate",
        statsTableStopHit: "Stop-hit rate",
        statsTableMfe30: "Avg MFE 30m",
        statsTableMae30: "Avg MAE 30m",
        statsTableSessionEnd: "Avg session-end net P&L",
  ...
  // the overlay; memoize it on its actual inputs.
    ...
    function SettingsSection(props) {
      ...
      var statsNet = function (value) {
        if (!Number.isFinite(Number(value))) return "—";
        var num = Number(value);
        var cls = num >= 0 ? "dsh-goldboard-stats-net-pos" : "dsh-goldboard-stats-net-neg";
        return el("span", { className: cls }, (num >= 0 ? "+" : "") + num.toFixed(2));
      };
      ...
      return el("div", { className: "dsh-goldboard-settings" },
        ...
        el(CardRow, { title: format(t, "statsTitle"), hint: format(t, "statsHint") },
          ...
          statsReport && (statsReport.perAction || []).length > 0 ? el("div", { className: "dsh-goldboard-stats-scroll" },
            el("table", { className: "dsh-goldboard-stats-table" },
              el("thead", null, el("tr", null,
                el("th", null, format(t, "statsTableAction")),
                el("th", null, format(t, "statsTableCount")),
                el("th", null, format(t, "statsTableTargetHit")),
                el("th", null, format(t, "statsTableStopHit")),
                el("th", null, format(t, "statsTableMfe30")),
                el("th", null, format(t, "statsTableMae30")),
                el("th", null, format(t, "statsTableSessionEnd")),
              )),
              el("tbody", null, statsReport.perAction.map(function (row) {
                return el("tr", { key: row.action },
                  ...
                  el("td", null, statsNet(row.sessionEndAvgNet)),
                );
              })),
            ),
          ) : null,
          statsReport && (statsReport.perAction || []).length === 0 && !statsRunning ? el("div", { className: "dsh-goldboard-stats-empty" }, format(t, "statsNoEvents")) : null,
