  ...
  factory: function (require) {
    ...
    var DICT = {
      zh: {
        ...
        analysisTitle: "模型与分析",
        analysisHint: "使用 Harness 已配置的模型解释已完成的指标与规则结果；模型不能绕过数据质量门控或生成交易指令。",
      ...
      en: {
        ...
        analysisHint: "Uses a configured Harness model to explain completed indicators and rule results. The model cannot bypass data-quality gates or issue trading instructions.",
    ...
    function filterTodayBars(bars, serverTime, tradingHours) {
      ...
      return bars.filter(function (bar) {
        return beijingDateKey(bar && bar.t) === day && isTradingMinute(beijingMinutes(bar && bar.t), tradingHours);
      });
    }
    ...
    function fillMissingChartSlots(bars, serverTime, midnightPrice) {
      ...
      for (var minute = 0; minute < firstMinute; minute += 1) {
        var earlyT = midnight + minute * 60_000;
        if (byTime[earlyT]) result.push(byTime[earlyT]);
      }
    ...
    function useSnapshot(localeHint) {
      ...
      useEffect(function () {
        ...
        var timer = null;
        var load = function () {
          fetch(SNAPSHOT_URL, { headers: { accept: "application/json", "x-dsh-locale": localeHint === "en" ? "en" : "zh" } })
      ...
      return el("div", { className: "dsh-goldboard-settings" },
        ...
        el(CardRow, {
          ...
          analysisResult && analysisResult.ok === true && analysisResult.analysis ? el("div", { className: "dsh-goldboard-analysis-result" },
            el("div", { className: "dsh-goldboard-analysis-result-row" }, el("span", { className: "dsh-goldboard-analysis-result-label" }, format(t, "analysisBias")), el("span", null, analysisEnumLabel(t, "Bias", analysisResult.analysis.bias))),
            el("div", { className: "dsh-goldboard-analysis-result-row" }, el("span", { className: "dsh-goldboard-analysis-result-label" }, format(t, "analysisAction")), el("span", null, analysisEnumLabel(t, "Action", analysisResult.analysis.action))),
            el("div", { className: "dsh-goldboard-analysis-result-row" }, el("span", { className: "dsh-goldboard-analysis-result-label" }, format(t, "analysisHorizon")), el("span", null, analysisEnumLabel(t, "Horizon", analysisResult.analysis.horizon))),
            el("div", { className: "dsh-goldboard-analysis-result-row" }, el("span", { className: "dsh-goldboard-analysis-result-label" }, format(t, "analysisConfidence")), el("span", null, analysisEnumLabel(t, "Confidence", analysisResult.analysis.confidence))),
            el("div", { className: "dsh-goldboard-analysis-result-row" }, el("span", { className: "dsh-goldboard-analysis-result-label" }, format(t, "analysisEvidence")), el("span", null, (analysisResult.analysis.evidence || []).join(" · ") || "—")),
            ...
            (analysisResult.analysis.dataWarnings || []).length > 0 ? el("div", { className: "dsh-goldboard-analysis-result-row" }, el("span", { className: "dsh-goldboard-analysis-result-label" }, format(t, "analysisWarnings")), el("span", null, analysisResult.analysis.dataWarnings.join(" · "))) : null,
          ) : null,
