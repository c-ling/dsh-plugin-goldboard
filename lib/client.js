      ...
      var channelCard = function (key, title, hasSecret) {
        ...
        return el(CardRow, { key: key, title: title, hint: format(t, "webhookTemplateHint") },
          ...
          hasSecret ? el("div", { className: "dsh-goldboard-row" },
            ...
            secretSet["webhooks." + key + ".secret"] ? el("span", { className: "dsh-goldboard-tag dsh-goldboard-tag-ok" }, format(t, "webhookSecretBadge")) : null,
            secretSet["webhooks." + key + ".secret"] ? el("button", {
              className: "dsh-goldboard-btn",
              type: "button",
              onClick: function () {
                var dotted = "webhooks." + key + ".secret";
                update(function (next) { if (next.webhooks[key]) next.webhooks[key].secret = ""; });
                setClearSecrets(function (next) { return next.includes(dotted) ? next : next.concat(dotted); });
              },
