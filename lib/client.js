  ...
  factory: function (require) {
    ...
    var DICT = {
      zh: {
        ...
        statsTitle: "策略统计",
        statsHint: "对最近 N 个交易日按当前策略参数做批量回放统计（只读分析，不发提醒、不调模型）：以 Au99.99 信号道逐 5 分钟重建指标与规则，统计各动作触发次数与事后命中情况；confirmBars/冷却照常生效。同参数结果缓存 1 小时。",
        statsDays: "统计天数",
        statsGenerate: "生成统计",
        ...
        caveat_lane_au9999_only: "统计仅在 Au99.99 信号道进行：招行实时报价没有历史 K 线，结论不能直接套用于招行实盘口径。",
        ...
        caveat_history_depth_limited: "免费源历史深度有限（5m 约 10+ 个交易日），超出深度的天数会在 failures 中如实列出。",
      ...
      en: {
        ...
        statsHint: "Batch replay of the last N trading days under the current strategy parameters (read-only analysis — no alerts, no model calls): indicators and rules are rebuilt bar-by-bar on the Au99.99 signal lane, reporting how often each action fired and how it played out afterwards; confirmBars/cooldown stay active. Same-parameter results are cached for 1 hour.",
        statsDays: "Days",
        ...
        caveat_lane_au9999_only: "Statistics run on the Au99.99 lane only: live CMB quotes have no history, so conclusions do not transfer 1:1 to the CMB desk.",
        ...
        caveat_history_depth_limited: "Free-source history is shallow (about 10+ trading days of 5m bars); days beyond that depth are listed honestly in failures.",
