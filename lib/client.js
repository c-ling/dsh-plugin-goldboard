  ...
  factory: function (require) {
    ...
    var DICT = {
      zh: {
        ...
        statsTitle: "策略统计",
        statsHint: "对最近 N 个交易日按当前策略参数逐 5 分钟回放重建（只读分析：不发提醒、不调模型）。顶部连续账户从 0 克开始，按时间顺序执行买入、补仓和卖出/减仓建议，使用当前最大克数、手续费和滑点，并对期末未卖出持仓按可卖价估值。下方表格仅用于观察单类信号的独立样本质量，不能与顶部累计收益相加。修改参数后重新生成会自动重算；数据不足的天数会如实跳过。",
        statsDays: "统计天数",
        statsGenerate: "生成统计",
        statsGenerating: "正在生成统计…（逐日回放中，请稍候）",
        statsCached: "（缓存）",
        statsEmpty: "暂无报告，点击「生成统计」开始。",
        statsLegacyReport: "此报告使用旧的独立信号合计口径，不能代表从 0 克开始的连续账户收益。请重新生成统计。",
        ...
        statsCardTotalNet: "期末模拟净收益（元）",
        statsCardTotalNetHint: "从 0 克开始，按时间顺序执行全部模拟成交；期末剩余持仓按最后可卖价和卖出费用估值。",
        ...
        statsDetailsHint: "每行是一笔按策略实际模拟成交的操作。买入现金流为负，卖出现金流为正；已实现收益只在卖出时产生。",
        ...
        statsDetailsTipTime: "信号在该根 5 分钟 K 线收盘后确认并按此时模拟成交。",
        ...
        statsTableTargetHit: "目标命中率",
        statsTableStopHit: "止损触发率",
        ...
        statsConfidenceTitle: "置信分与目标命中率（入场事件）",
        ...
        caveat_continuous_zero_position: "顶部连续账户从 0 克开始，按时间顺序执行策略建议；期末未卖出持仓以最后可卖价扣除卖出手续费和滑点估值。它是模拟结果，不代表实际成交。",
        caveat_two_simulated_passes: "每天按「空仓」与「持仓（按当日开盘价建仓）」两条模拟通道各评估一次，动作计数为两条通道合计。",
        caveat_past_performance_advisory: "历史命中率不代表未来表现；本报告仅为策略调参参考，非投资建议。",
      ...
      en: {
        ...
        statsTitle: "Strategy statistics",
        statsHint: "Batch replay of the last N trading days under the current strategy parameters, bar-by-bar (read-only: no alerts, no model calls). The top continuous account starts from 0 g, applies buy, add, sell, and trim suggestions in time order under the current maximum grams, fees, and slippage, then values any unsold holding at the final sell price. The tables below only measure independent sample quality for each signal family and must not be added to the account total. Regenerate after changing parameters; days without enough data are skipped honestly.",
        statsDays: "Days",
        ...
        statsLegacyReport: "This report uses the older independent-signal aggregate and does not represent a continuous account starting from 0 g. Generate the statistics again.",
        ...
        statsCardTotalNet: "End-of-window simulated net P&L (CNY)",
        statsCardTotalNetHint: "Starts from zero grams and fills every simulated order in time order; any remaining position is valued at the final sell price after exit costs.",
        ...
        statsDetailsHint: "Each row is a simulated trade actually filled by the strategy. Buy cash flow is negative and sell cash flow is positive; realized P&L exists only on sales.",
        ...
        statsDetailsTipTime: "The signal was confirmed after this 5-minute bar closed and filled at this simulated time.",
        ...
        statsTableTargetHit: "Target hit rate",
        statsTableStopHit: "Stop-hit rate",
        ...
        statsConfidenceTitle: "Confidence score vs target hit rate (entry events)",
        ...
        caveat_continuous_zero_position: "The top continuous account starts at 0 g and fills strategy suggestions in time order. Holdings left at the end are valued at the final sell price after exit fee and slippage. This is a simulation, not actual execution.",
        caveat_two_simulated_passes: "Each day is evaluated twice — flat and holding (a position opened at that day's open price); action counts sum both passes.",
        caveat_past_performance_advisory: "Past hit rates do not predict future results; this report exists to inform parameter tuning and is not investment advice.",
        ...
        fetchJson(REPLAY_STATS_URL + "?detail=true")
          .then(function (body) {
            if (!body || body.ok !== true) {
              throw new Error((body && body.error && (body.error.message || body.error.code)) || "bad response");
            }
            setTrades(Array.isArray(body.trades) ? body.trades : null);
            setSummary(body.report && typeof body.report.overall === "object" ? body.report.overall : null);
          })
          .catch(function (err) {
            setError(String(err && err.message ? err.message : err));
          })
          ...
          Array.isArray(trades) && trades.length > 0 ? el("div", { className: "dsh-goldboard-stats-scroll" },
            el("table", { className: "dsh-goldboard-stats-table" },
              ...
              el("tbody", null, trades.map(function (trade, index) {
                return el("tr", { key: (trade && trade.t) + "-" + (trade && trade.action) + "-" + index },
                  el("td", null, formatBeijingTime(trade && trade.t, false)),
                  el("td", null, actionLabel(trade && trade.action)),
                  el("td", null, fmtPrice(trade && trade.price)),
                  el("td", null, fmtPrice(trade && trade.grams)),
    ...
    function SettingsSection(props) {
      ...
      var saveViaSettings = function () {
        if (!settingsApi || !settingsApi.settings || typeof settingsApi.settings.mutate !== "function") {
          reportSaveFailure("settings transport unavailable");
          return;
        }
        ...
        settingsApi.settings.mutate(request)
          .then(function (response) {
            ...
            if (result.ok) {
              ...
              reportSaveFailure((result.error && result.error.message) || "settings write failed");
            }
          })
          .catch(function (err) {
            setSaving(false);
        ...
        fetchJson(CONFIG_URL, {
          ...
          .then(function (body) {
            ...
            if (body && body.ok) {
              ...
              reportSaveFailure((body && body.error && body.error.message) || "bad response");
            }
          })
          .catch(function (err) {
            setSaving(false);
        ...
        fetchJson(REPLAY_STATS_URL, {
          ...
          .then(function (body) {
            if (body && body.ok && body.report) {
              ...
              var detail = (body && body.error && (body.error.message || body.error.code)) || "bad response";
              setStatsMessage(format(t, "statsFailed", { error: detail }));
            }
          })
          .catch(function (err) {
            setStatsMessage(format(t, "statsFailed", { error: String(err && err.message ? err.message : err) }));
          })
      ...
      var statsPct = function (rate) {
        return Number.isFinite(Number(rate)) ? Math.round(Number(rate) * 1000) / 10 + "%" : "—";
      };
      ...
      var statsOverallFallback = function (report) {
        var tracked = (Array.isArray(report.perAction) ? report.perAction : [])
          .filter(function (row) { return Number.isFinite(Number(row.sessionEndAvgNet)); });
        var countSum = tracked.reduce(function (sum, row) { return sum + row.count; }, 0);
        var isEntryRow = function (row) { return row.action === "buy_setup" || row.action === "add_position"; };
        return {
          eventsWithOutcome: countSum,
          entryEvents: tracked.filter(isEntryRow).reduce(function (sum, row) { return sum + row.count; }, 0),
          exitEvents: tracked.filter(function (row) { return !isEntryRow(row); }).reduce(function (sum, row) { return sum + row.count; }, 0),
          winRate: null,
          avgNetPerGram: countSum > 0 ? tracked.reduce(function (sum, row) { return sum + row.count * row.sessionEndAvgNet; }, 0) / countSum : null,
          totalNetCny: null,
          avgNetCnyPerEvent: null,
        };
      };
      var statsOverallOf = function (report) {
        if (!report || !report.overall || report.overall.accounting !== "continuous-zero-position") return null;
      ...
      var genericCards = cfg.webhooks.generic.map(function (item, index) {
        return el("div", { className: "dsh-goldboard-generic", key: item.id },
          webhookCard({
            key: "generic-" + item.id,
            title: item.name || item.id || format(t, "genericAdd"),
            channel: item,
      ...
      return el("div", { className: "dsh-goldboard-settings" },
        ...
        el(CardRow, { title: format(t, "statsTitle"), hint: format(t, "statsHint") },
          ...
          statsReport && !statsOverallOf(statsReport) ? el("div", { className: "dsh-goldboard-hint" }, format(t, "statsLegacyReport")) : null,
          (function () {
            // v4 account total: one chronological portfolio starts at zero
            // grams. Legacy independent-event reports are intentionally not
            // relabelled as a continuous equity result.
            ...
            return el("div", { key: "overall" },
              ...
              el("div", { className: "dsh-goldboard-stats-overall" },
                statsCard("statsCardTotalNet", ov.totalNetCny === null || ov.totalNetCny === undefined ? "—" : statsSignedValue(ov.totalNetCny), null,
                  el("button", {
                ...
                statsCard("statsCardUnrealizedNet", ov.unrealizedNetCny === null || ov.unrealizedNetCny === undefined ? "—" : statsSignedValue(ov.unrealizedNetCny)),
                statsCard("statsCardEndingPosition", fmtPrice(ov.endingGrams)),
              ),
            );
          })(),
          (function () {
            // Parameter snapshot (v3 reports): shows exactly which strategy
          ...
          statsReport && (statsReport.confidenceBuckets || []).length > 0 ? el("div", { className: "dsh-goldboard-stats-summary" },
            el("strong", null, format(t, "statsConfidenceTitle")),
            statsReport.confidenceBuckets.map(function (bucket) {
