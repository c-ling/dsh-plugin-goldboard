  ...
  factory: function (require) {
    ...
    var DICT = {
      zh: {
        ...
        statsDetails: "明细",
        statsDetailsTitle: "连续账户成交明细",
        statsDetailsHint: "每行是一张模拟限价单及其状态；只有下一根及后续可执行 K 线触及限价且未过期时才记为模拟成交。",
        statsDetailsRefresh: "刷新",
        statsDetailsLoading: "正在读取成交明细…",
        statsDetailsLoadError: "无法读取成交明细：{error}",
        statsDetailsEmpty: "当前报告没有可显示的模拟成交。请确认最大克数和策略信号条件。",
        statsDetailsUnavailable: "该旧报告未保存成交明细。请重新生成统计后再查看。",
        statsDetailsTime: "时间",
      ...
      en: {
        ...
        statsDetailsTitle: "Continuous account trade details",
        statsDetailsHint: "Each row is a simulated limit order and its status. A simulated fill occurs only when a later eligible bar touches the limit before expiry.",
        statsDetailsRefresh: "Refresh",
        statsDetailsLoading: "Loading trade details…",
        statsDetailsLoadError: "Unable to load trade details: {error}",
        statsDetailsEmpty: "This report has no simulated trades. Check the maximum grams and strategy signal conditions.",
        statsDetailsUnavailable: "This older report has no saved trade details. Generate the statistics again, then reopen details.",
    ...
    function ReplayStatsDetailsDialog(props) {
      ...
      return el(ModalDialog, {
        ...
        el("div", { className: "dsh-goldboard-stats-detail-body", "aria-live": "polite" },
          loading && trades === null ? el("div", { className: "dsh-goldboard-loading" }, format(t, "statsDetailsLoading")) : null,
          error ? el("div", { className: "dsh-goldboard-error" }, format(t, "statsDetailsLoadError", { error: error })) : null,
          !loading && !error && trades === null ? el("div", { className: "dsh-goldboard-stats-empty" }, format(t, "statsDetailsUnavailable")) : null,
          !loading && !error && Array.isArray(trades) && trades.length === 0 ? el("div", { className: "dsh-goldboard-stats-empty" }, format(t, "statsDetailsEmpty")) : null,
          Array.isArray(trades) && trades.length > 0 ? el("div", { className: "dsh-goldboard-stats-scroll" },
